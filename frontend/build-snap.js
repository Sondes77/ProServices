...
const MAX_URLS_PER_FILE = 5000; // safe pour react-snap
const OUTPUT = path.join(__dirname, "reactSnapUrls.json");

/*
|--------------------------------------------------------------------------
| SLUGIFY SEO (TRÈS IMPORTANT)
|--------------------------------------------------------------------------
| transforme :
| "Maçon" -> macon
...
/*
|--------------------------------------------------------------------------
| VILLES (extrait — garde ta liste complète ici)
|--------------------------------------------------------------------------
*/
...
metiers.forEach(m => {
  ...
  // page métier
  urls.add(`/services/${mSlug}`);

  Object.values(villes).forEach(list => {
    list.forEach(v => {
      const vSlug = slugify(v);

      urls.add(`/services/${mSlug}/${vSlug}`);
    });
  });
});
...
/*
|--------------------------------------------------------------------------
| FRACTIONNEMENT (IMPORTANT SI > 10K URLS)
|--------------------------------------------------------------------------
*/
...
const chunks = [];
for (let i = 0; i < urlArray.length; i += MAX_URLS_PER_FILE) {
  chunks.push(urlArray.slice(i, i + MAX_URLS_PER_FILE));
}
...
console.log(`✅ ${urlArray.length} URLs générées pour react-snap`);
console.log(`📄 fichier: reactSnapUrls.json`);